...
export class LichSanController {
  ...
  @Get('lich-trong')
@ApiOperation({
  summary: 'Lấy danh sách lịch sân trống sinh viên có thể đặt',
})
async getLichTrongChoSinhVien(): Promise<ApiResponse<LichSan[]>> {

  const data = await this.lichSanService.findLichTrongChoSinhVien();

  return successResponse(
    data,
    'Lấy danh sách lịch trống sinh viên có thể đặt thành công',
  );
}
